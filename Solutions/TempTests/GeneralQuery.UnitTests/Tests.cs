...
using WitsWay.TempTests.DynamicQuery.Editors;
...
namespace WitsWay.TempTests.GeneralQuery.UnitTests
{
    ...
    public class Tests
    {
        ...
        {
            [RightField]
            [InputEditor(EditorKind = EditorKinds.SpinEdit)]
            [DisplayName("年龄")]
        ...
        {
            var result = typeof(ColumnBuilderTestClass).GetDefaultColumnDefinitions();
            Assert.IsTrue(result.Count == 8);

            result = typeof(ColumnBuilderTestClass).GetDefaultColumnDefinitions(true);
