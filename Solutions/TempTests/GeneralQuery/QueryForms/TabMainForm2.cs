...
{
    public partial class TabMainForm2 : DevExpress.XtraBars.TabForm
    {
        public TabMainForm2()
        {
            InitializeComponent();
            _navBarControl.OptionsNavPane.ShowOverflowButton = false;
            _navBarControl.OptionsNavPane.ShowOverflowPanel = false;
            //_navBarControl.OptionsNavPane.MaxPopupFormWidth = 400;
            _navBarControl.OptionsNavPane.AllowOptionsMenuItem = false;
        }
        ...
        {
            var visible = _navBarControl.Visible;
            _navBarControl.Visible = !visible;
        ...
        private void _tabFormControl_SelectedPageChanged(object sender, TabFormSelectedPageChangedEventArgs e)
        {
            if (e.Page == null) return;
            SuspendLayout();
            var tagName = e.Page.Tag as string;
            if (string.IsNullOrWhiteSpace(tagName))
            {
                var frm = new GridWithFilterUc { Dock = DockStyle.Fill };
                e.Page.ContentContainer.Controls.Add(frm);
                e.Page.Tag = frm.Name;
            }e.Page.ContentContainer.Padding = new Padding(_navBarControl.Width + 5, 0, 0, 0);
            ResumeLayout(false);
        }

        private void TabMainForm2_Load(object sender, EventArgs e)
        {
            _navBarControl.OptionsNavPane.PopupFormSize = new Size(0, this.Height - 15);//_navBarControl.Height - 40);
        }

        private void TabMainForm2_SizeChanged(object sender, EventArgs e)
        {
            _navBarControl.OptionsNavPane.PopupFormSize = new Size(0, this.Height - 15);
        }
