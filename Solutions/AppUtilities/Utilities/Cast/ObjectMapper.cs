...
namespace WitsWay.Utilities.Cast
{
    /// <summary>
    /// 对象映射器类
    ...
    public class ObjectMapper
    {
        ...
        public static TTarget Map<TSource, TTarget>(TSource sourceObject, Action<TSource, TTarget> customeConvertAction = null) where TTarget : new()
        {
            ...
            if (customeConvertAction != null)
                customeConvertAction(sourceObject, result);
        ...
        public static IEnumerable<TTarget> Map<TSource, TTarget>(IEnumerable<TSource> sourceObjectList, Action<TSource, TTarget> customeConvertAction = null) where TTarget : new()
        {
        ...
        /// <param name="sourceObject">源对象</param>
        /// <returns>目标对象</returns>
        private static TTarget ConvertObject<TSource, TTarget>(object sourceObject)
            where TTarget : new()
        {
            ...
            foreach (var targetPropInfo in targetTypePropertiesDictionary.Values)
            {
                ...
                if (targetPropInfo.CanWrite && sourceTypePropertiesDictionary.TryGetValue(targetPropInfo.Name, out sourcePropInfo)
        ...
        {
            Dictionary<string, PropertyInfo> sourceTypeProperties;
            if (!TypePropertiesDictionary.TryGetValue(sourceType, out sourceTypeProperties))
            {
